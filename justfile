...
# Shebang recipes only receive their parameters as "$@" with this set, which is
# what `agentcfg *args` below relies on.
...
# Build the runtime image. The Dockerfile hardcodes `-p agent-server` and takes
# no build args, so this takes none either — a --build-arg the Dockerfile never
# declares is silently ignored, which is worse than not passing it.
docker-build:
    docker build --tag agent-server:latest .
